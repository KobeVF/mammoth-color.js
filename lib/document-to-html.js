...
function DocumentConversion(options, comments) {
    ...
    function convertToHtml(document) {
        ...
        return promises.mapSeries(deferredNodes, function(deferred) {
            ...
            // html has no indentation here
            Html.write(writer, Html.simplify(replaceDeferred(html))); // Writer, nodes (dont have indent)
    ...
    function convertParagraph(element, messages, options) {
        return htmlPathForParagraph(element, messages).wrap(function() {
            console.log("INDENT: " + JSON.stringify(element.indent));
            for (var iter = 0; iter < element.children.array.length; ++iter) {
                element.children[iter].indent = element.indent;
            }
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.color) {
            // TODO: Add all elemt types we (might sometime) need
            paths.push(htmlPaths.element('p', {color : run.color}, {fresh: false}));
        }
        if (run.indent) {
            // TODO: Add all elemt types we (might sometime) need
            paths.push(htmlPaths.element('p', {indent : run.indent}, {fresh: false}));
            //paths.push(htmlPaths.element('p', {color : run.color}, {fresh: false}));
        }
